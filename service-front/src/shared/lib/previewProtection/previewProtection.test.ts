...
describe('requireBasicAuth', () => {
    ...
    it('除外パス（プレフィックス一致）は Authorization が無くても通過する', () => {
        const excluded = ['/api/stripe/webhook'];

        expect(requireBasicAuth(requestTo('/api/stripe/webhook'), CREDENTIALS, excluded)).toBeNull();
        expect(requireBasicAuth(requestTo('/api/stripe/webhook/'), CREDENTIALS, excluded)).toBeNull();
        expect(requireBasicAuth(requestTo('/api/stripe'), CREDENTIALS, excluded)?.status).toBe(401);
    });
});
