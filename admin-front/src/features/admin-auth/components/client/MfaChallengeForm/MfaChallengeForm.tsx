...
import { challengeAdminLoginFactor, verifyAdminLogin } from '@/features/admin-auth/server/mfaActions';

/** SMS ワンタイムコードの桁数（supabase/config.toml [auth.mfa.phone] otp_length と一致させる） */
const OTP_LENGTH = 6;
const OTP_PATTERN = new RegExp(`^\\d{${OTP_LENGTH}}$`);
/** 再送のクールダウン秒数（SMS の連打抑止） */
...
/**
 * 管理者ログイン 2 段階目の SMS コード入力フォーム。
 * まず「送信」で SMS を送り、届いたコードを入力して verifyAdminLogin で AAL2 へ昇格する。
...
export const MfaChallengeForm = ({ factorId }: MfaChallengeFormProps) => {
    ...
    const handleSendCode = () => {
        ...
        startTransition(async () => {
            const result = await challengeAdminLoginFactor(factorId);
            if (!result.success) {
    ...
    const handleVerify = () => {
        ...
        startTransition(async () => {
            const result = await verifyAdminLogin(factorId, challengeId, code);
